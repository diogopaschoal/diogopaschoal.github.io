...
1. **Split layout**: The blog list page is divided into two regions.
   ...
   - _Acceptance_: Left region uses approximately **30%** of the page width; right region uses approximately **70%**. Left shows the list of posts; right shows the selected post (article) content.
...
- **Responsiveness**: Layout (30% / 70%) applies at desktop widths; implementation may stack or adapt for narrow viewports (e.g. list above article, or list in a drawer).
...
- Use CSS (e.g. flexbox or grid) or a layout component to enforce the 30% / 70% split; avoid fixed pixel widths so the layout works across screen sizes.
